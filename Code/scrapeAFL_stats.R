...
scrapeplayer <- function() {
    ...
    for (i in 1:nSeasons) {
        ...
        for (j in 1:nTeams) {
            ...
            xrecord <- scrapeplayer_team_season(team,team.code,season)
...
scrapeplayer_team_season <- function(team,team.code,season) {
    ...
    for (i in 1:nMoves) {
        ...
        if (!is.na(strStart)) {
            ...
            df$move <- abbr
            df <- as.data.frame(cbind(season,team.code,df))
            playerStats <- rbind(playerStats,df)
        }
    }
    
    names(playerStats) <- c("Season","Team","Name","Round","StatCount","Stat")
    playerStats
    
    ## fix Josh Kennedy in WC (west coast) and SY (sydney swans)
    ## lstJosh <- playerStats$Name == "Kennedy, Josh"
    ## lstSY <- playerStats$Team == "SY"
    ## lstFind1 <- lstJosh & lstSY
    ## lstFind2 <- lstJosh & !lstSY
    ## playerStats[lstFind1,]
    }
